...
/** Native dimensions of the public/preview/model-*.png source photos (the AI
 * try-on render keeps this same framing, per the prompt in lib/aiTryOn.js). */
const MODEL_PHOTO_DIMENSIONS = {
  woman: {
    front: { width: 682, height: 1024 },
    back: { width: 576, height: 1024 },
  },
  man: {
    front: { width: 682, height: 1024 },
    back: { width: 682, height: 1024 },
  },
};

export function modelPhotoDimensions(bodyType, view = 'front') {
  const body = bodyType === 'man' ? 'man' : 'woman';
  const angle = view === 'back' ? 'back' : 'front';
  return MODEL_PHOTO_DIMENSIONS[body][angle];
}
...
const CATEGORY_AI_LABEL = {
  tops: 'top garment',
  shirts: 'top garment',
  'chef-wear': 'chef jacket (top)',
  'spa-wear': 'top garment',
  epaulettes: 'epaulette shoulder insignia',
  engineering: 'coverall/jumpsuit (full body)',
  dresses: 'dress (full body)',
  bottoms: 'bottom garment (trousers/shorts)',
  outerwear: 'outerwear/jacket',
  shoes: 'pair of shoes',
  accessories: 'accessory',
};

/** Human-readable label describing a garment's body region, for the AI try-on prompt. */
export function garmentAiLabel(product) {
  return CATEGORY_AI_LABEL[product?.category] || 'garment';
}
