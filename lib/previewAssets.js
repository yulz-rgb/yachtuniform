...
export const FIGURE_DIMENSIONS = {
  woman: {
    front: { width: 294, height: 983 },
    back: { width: 289, height: 981 },
  },
  man: {
    front: { width: 312, height: 985 },
    back: { width: 316, height: 989 },
  },
};
...
export function deckPanelSrc() {
  return '/preview/yacht-deck-panel.jpg';
}
...
export function resolveGarmentHint(product) {
  ...
  if (/\b(trouser|trousers|chino pant|chino pants|\bpant\b|\bpants\b|jogger|legging)\b/i.test(name)
    && !/\b(short|skort|bermuda)\b/i.test(name)) return 'trousers';
  if (/\b(short|skort|bermuda)\b/i.test(name)) return 'shorts';
...
const HINT_OVERRIDES = {
  ...
  shoes: { fit: 'contain', scale: 1.12, objectPosition: '50% 75%' },
...
export function garmentLayers(bodyType, view, selectedProducts = []) {
  return resolvePreviewProducts(selectedProducts, bodyType)
    .map((product) => ({ product, slot: slotForProduct(product, bodyType, view) }))
